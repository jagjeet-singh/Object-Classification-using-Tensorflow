...
def cnn_model_fn(features, labels, mode):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        input_aug = tf.map_fn(fn=rand_flip, elems=input_layer, name='my_rotate')

    elif mode == tf.estimator.ModeKeys.PREDICT:
        input_aug = tf.map_fn(fn=rand_crop, value=input_layer, size=[128,128,3], name='my_Crop')
    ...
    tf.summary.scalar('random',np.random.random_sample())
...
def main(unused_argv):
    ...
    mnist_classifier = tf.estimator.Estimator(
        model_fn=cnn_model_fn, model_dir="mnistParams")
    # embed()
